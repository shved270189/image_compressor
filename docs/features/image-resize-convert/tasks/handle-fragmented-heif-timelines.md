---
id: "T4"
title: "Handle fragmented HEIF timelines"
layer: "domain"
deps: ["T3"]
blocks: ["T5"]
acs: ["AC-10", "AC-12"]
...
context_budget: "M"
status: "todo"
dod: "tests/test_images.py fragmented/edit/composition and decodable-gallery checks pass on macOS and Linux; no production NEEDS_TIMELINE fallback remains."
file: "docs/features/image-resize-convert/tasks/handle-fragmented-heif-timelines.md"
---

# T4 — Handle fragmented HEIF timelines

## Place in the sequence

- **Blocked by:** T3 — Classify HEIF presentation timelines. **Blocks:** T5 — Resize and normalize output. **Wave:** 3 (after its prerequisites).
- **Lane:** Shares files with T2, T3, T5, T6, T7, T8, T12; serialize overlapping edits in task-number order after dependencies are ready.
...
Handle fragmented HEIF timelines delivers the slice defined in the checklist; shared ACs below are completed jointly with the other tasks listed in the epic.
...
- [ ] Complete fragmented timeline evaluation in backend/images.py, including its interaction with edits and composition timing. Remove diagnostic NEEDS_TIMELINE paths; never convert unresolved probe cases into static/animated defaults.
- [ ] Add independently generated decodable non-timed gallery and fragmented sequence controls to tests/fixtures/images; analytic timing structures alone are insufficient acceptance evidence.
- [ ] Extend tests/test_images.py to assert gallery decoding/primary selection and sequence rejection on macOS and Linux using the implementation, not only the audit script.

## Edge cases

| Case | Required behavior |
|---|---|
| Fragmented animated sequence | Reject based on actual presentation timeline. |
| Decodable non-timed gallery | Accept and export the designated static primary. |
| Unresolved diagnostic classification | Complete the required handling; never silently accept or reject using a fallback. |
...
- [ ] tests/test_images.py fragmented/edit/composition and decodable-gallery checks pass on macOS and Linux; no production NEEDS_TIMELINE fallback remains.
