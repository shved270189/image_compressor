...
Review follow-up 2026-09-06 (`CHANGES REQUESTED`): T9 restored the screens.md HEIC appearance disclaimer (AC-10); T11 recorded Chrome Preview 8×12 vs download 4×6 on `oriented-exif6.jpg` (AC-02, AC-09). Smoke pin `test_heic_notice_copy`. Gate: `npm --prefix frontend run build` PASS; `uv run pytest -q` 181 passed; `uv run ruff check .` PASS; `npm --prefix frontend run lint` PASS. Tasks remain done.
