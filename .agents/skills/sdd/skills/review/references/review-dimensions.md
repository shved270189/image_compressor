...
- **UI reuse (for a UI surface).** Composes the existing design system / components / tokens / styling (`architecture-map.md` §Frontend + the `docs/design-system.md` inventory) rather than reinventing — flag from-scratch UI that duplicates an existing primitive or introduces a second styling system. When `screens.md` exists, **the built screen matches the approved manifest**: every declared state is present, the named components are the ones reused, and any `NEW` component was registered back into `docs/design-system.md`.
